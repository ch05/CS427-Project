...
<%
	...
	//pageTitle = outbox?"iTrust - View My Sent Messages":"iTrust - View My Message";
	/* if (outbox){
...
/* 	
	...
	EditPatientAction p_action = new EditPatientAction(prodDAO, loggedInMID.longValue(), loggedInMID.toString()); 
*/
	...
	if (request.getParameter("edit") != null
		...
		if (request.getParameter("cancel") != null)
			response.sendRedirect(outbox ? "messageOutbox.jsp"
					: "messageInbox.jsp");
			...
			if (!error) {
				if (request.getParameter("test") != null) {
					response.sendRedirect(outbox ? "messageOutbox.jsp?edit=true&testFilter="+nf
							: "messageInbox.jsp?edit=true&testFilter="+nf);
					...
					if (isHCP){
						...
						EditPatientAction p_action = new EditPatientAction(prodDAO, loggedInMID.longValue(), loggedInMID.toString()); 
						p_action.editMessageFilter(nf);
					}
					response.sendRedirect(outbox ? "messageOutbox.jsp?filter=true" :"messageInbox.jsp?filter=true");
				}
			}
		}
		...
		if (request.getParameter("testFilter") != null) {
			...
			if (isHCP) {
				PersonnelDAO personnelDAO = new PersonnelDAO(prodDAO);
				filter = personnelDAO.getPersonnel(
						loggedInMID.longValue()).getMessageFilter();
			}
			else {
				PatientDAO patientDAO = new PatientDAO(prodDAO);
				filter = patientDAO.getPatient(loggedInMID.longValue())
						.getMessageFilter();
			}
			if (!filter.equals("")) {
				String[] f = filter.split(",", -1);
	...
	if (request.getParameter("sort") != null) {
		...
		sortHow = request.getParameter("sorthow");
		if (sortBy.equals("name")) {
			if (sortHow.equals("asce")) {
	...
	if ((request.getParameter("filter") != null && request
			...
			|| (request.getParameter("testFilter") != null && request
					.getParameter("testFilter").equals("true"))) {
		String filter = "";
		if (request.getParameter("testFilter") != null) {
		...
		if (!filter.equals("") && !filter.equals(",,,,,")) {
			List<MessageBean> filtered = action.filterMessages(
					messages, filter);
			messages = filtered;
			is_filtered = true;
		}
...
<div class="filterEdit">
	<div align="center">
		...
		<form method="post" 
		action="<%=outbox ? "messageOutbox.jsp?edit=true"
					: "messageInbox.jsp?edit=true"%>">
			<table>
...
%>
<form method="post"
	action="<%=outbox ? "messageOutbox.jsp"
							+ StringEscapeUtils.escapeHtml("" + (is_filtered ? "?filter=true" : ""))
						: "messageInbox.jsp"
							+ StringEscapeUtils.escapeHtml("" + (is_filtered ? "?filter=true" : ""))%>">
	<table>
		<tr> 
		...
		<tr> 
			...
			if (editing == false) {
			%>
			<td><a href="<%=outbox ? "messageOutbox.jsp?edit=true"
					: "messageInbox.jsp?edit=true"%>">Edit Filter</a></td>
			<%
				}
			%>
			<td><a href="<%=outbox ? "messageOutbox.jsp?filter=true"
					: "messageInbox.jsp?filter=true"%>">Apply Filter</a></td>
		</tr>
	</table>
</form>
<br />
