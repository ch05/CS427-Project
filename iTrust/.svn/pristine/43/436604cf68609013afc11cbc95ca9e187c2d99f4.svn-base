...
FeatureDAO feaDAO  = prodDAO.getFeatureDAO();
...
List<FeatureBean> features = feaDAO.getFeatures();

for (FeatureBean feature : features){
	if (feature.getFilePath().equals(requestedURI) && !feature.isEnabled()){
		response.sendRedirect("/iTrust/unavailable.jsp");
	}
}
