...
public class ResponseDAO {
	...
	/**
	 * Returns the average response latency.
	 * 
	 * @return an Integer denoting the average latency in milliseconds
	 * @throws DBException
	 */
	public int getAverageLatency() throws DBException{
		Connection conn = null;
		PreparedStatement ps = null;
		int average = -1;
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT AVG(latency) AS avg_latency FROM response");
			ResultSet rs = ps.executeQuery();
			
			if (rs.next()){
				average = rs.getInt("avg_latency");
			}
			rs.close();
			ps.close();
	...
	/**
	 * Returns the average content length of a http response.
	 * 
	 * @return an Integer denoting the average content length in bytes
	 * @throws DBException
	 */
	public int getAverageContentLength() throws DBException{
		...
		try {
			conn = factory.getConnection();
			ps = conn.prepareStatement("SELECT AVG(content_length) AS avg_length FROM response");
			ResultSet rs = ps.executeQuery();
